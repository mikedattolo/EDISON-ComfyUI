...
async def chat(request: ChatRequest):
    ...
    if not llm_fast and not llm_deep:
        raise HTTPException(
            status_code=503,
            detail="No LLM models loaded. Please place GGUF models in models/ directory."
        )
    
    # Determine which model to use based on mode
    mode = request.mode
    if mode == "auto":
        # Use fast model for simple chat, deep model for complex tasks
        if len(request.message) > 200 or "explain" in request.message.lower():
            mode = "reasoning"
        else:
            mode = "chat"
